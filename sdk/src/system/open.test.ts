...
describe('open and locked systems', () => {
  it('consolidates without a style scope or side effects and exposes four identities', () => {
    ...
    expect(contract.format).toBe('vanity.introspection/2')
  ...
  it('keeps utility-only plugins out of value compatibility and labels direct constructors as system-owned', () => {
    const source = createSystem()
    const module = source.defineTokens({ color: { brand: 'red' } })
    const utilityPlugin = definePlugin({
      id: 'org.vanity.test.utility-only',
      version: 1,
      setup: ds => ds.addUtils({ spacing: (value: number) => `${value}px` }),
    })

    const withUtility = source.addPlugin(utilityPlugin)
    expect(() => withUtility.addTokens(module).consolidate()).not.toThrow()
    expect(withUtility.consolidate().introspect().extensions).not.toHaveProperty(utilityPlugin.id)

    const withConstructor = createSystem().addConstructor('tone', {
      call: (value: number) => source.length.px(value),
    }).consolidate()
    expect(withConstructor.introspect().constructors.tone.origin).toEqual({ kind: 'system' })
    expect(withConstructor.introspect().extensions).not.toHaveProperty('org.vanity.constructor.tone')
  })

  it('reuses token modules across equal policies with shared or cloned references', () => {
    const sharedPolicy = { nested: { intent: 'layout' } }
    const shared = createSystem({ alpha: sharedPolicy, beta: sharedPolicy })
    const cloned = createSystem({
      alpha: { nested: { intent: 'layout' } },
      beta: { nested: { intent: 'layout' } },
    })
    const module = shared.defineTokens({ color: { brand: 'red' } })

    expect(() => cloned.addTokens(module).consolidate()).not.toThrow()
  })
  ...
  it('validates portable data and its projection-derived identities at the trust boundary', () => {
    const ds = createSystem()
      ...
      .consolidate()
    const portable = getSystemContract(ds)!.portable
  ...
  it('rejects truncated, legacy, unknown, and semantically invalid portable artifacts at the boundary', () => {
    ...
    const { ruleGroups: _ruleGroups, ...withoutRuleGroups } = portable
    ...
    })).toThrow(/ruleGroups\[0\]\.stale.*not a property/)
    expect(() => assertPortableSystem({
      ...portable,
      capabilities: {
        ...portable.capabilities,
        constructors: [{ name: 'brand', origin: { kind: 'unknown' } }],
      },
    })).toThrow(/origin\.kind.*builtin, system, plugin, or extension/)
    expect(() => assertPortableSystem({
      ...portable,
