...
export const VANITY_CSS_PARITY_LEDGER = Object.freeze({
  ...
  'CSS-V015': record({
    id: 'CSS-V015',
    api: 'boxShadow property lane',
    cssConcept: 'box-shadow',
    spec: BACKGROUNDS_3,
    typedGrammar: ['none', '<shadow>#', 'CSS-wide keywords'],
    rawGrammar: 'complete property string lane',
    ...
    escape: 'standard property spelling or raw declaration lane',
  ...
  'CSS-V016': record({
    ...
    coverage: 'typed-subset+raw',
    decision: 'Typed and discoverable across the entire relative-color family; the raw lane preserves future Color 5 grammar.',
  }),
  'CSS-V017': record({
    id: 'CSS-V017',
    ...
    rawGrammar: 'complete grid property string lane',
  ...
  'CSS-G003': record({
    ...
    spec: VALUES_4,
    typedGrammar: [],
    rawGrammar: 'round(), mod(), and rem() through typed raw numeric lanes',
  ...
  'CSS-G004': record({
    ...
    spec: VALUES_4,
    typedGrammar: [],
    rawGrammar: 'all Values 4 trigonometric and exponential functions through typed raw numeric lanes',
    inputs: ['raw CSS syntax with an explicitly selected numeric result type'],
    cssWideKeywords: 'not-applicable',
    outputType: 'author-selected CSS numeric type',
    invalid: ['unbalanced CSS token syntax'],
    ...
    fixtures: ['src/values/ir.test.ts'],
    coverage: 'planned',
    decision: 'Planned only after the dimensional algebra and diagnostics are spiked; raw typed values remain the current honest lane.',
  }),
  'CSS-G005': record({
    id: 'CSS-G005',
  ...
  'CSS-G006': record({
    ...
    rawGrammar: 'complete transform grammar through typed raw lanes and property strings',
  ...
  'CSS-G007': record({
    ...
    fixtures: ['src/css/styling-v2.test.ts'],
    coverage: 'raw-only',
  ...
  'CSS-G008': record({
    ...
    spec: SHAPES_2,
    typedGrammar: [],
    rawGrammar: 'shape(), path(), ray(), and basic-shape grammar through standard property/raw lanes',
    inputs: ['raw CSS syntax'],
    cssWideKeywords: CSS_WIDE,
  ...
  'CSS-G009': record({
    ...
    spec: VALUES_5,
    typedGrammar: [],
    rawGrammar: 'complete attr() grammar through an explicitly typed raw value lane',
  ...
  'CSS-G010': record({
    ...
    spec: ENV_1,
    typedGrammar: [],
    rawGrammar: 'complete env() grammar through an explicitly typed raw value lane',
    ...
    fixtures: ['src/values/ir.test.ts'],
    coverage: 'planned',
    decision: 'Planned typed helper for known UA variables plus an open-name overload; the raw lane remains canonical for future variables.',
  }),
  'CSS-G011': record({
    id: 'CSS-G011',
    ...
    fixtures: ['src/values/ir.test.ts'],
    coverage: 'planned',
    decision: 'Planned after an isolated inference spike; the Working Draft is still changing and the current typed raw lane is lossless.',
  }),
  'CSS-G012': record({
    id: 'CSS-G012',
    ...
    fallback: 'not applicable',
    escape: 'ds.raw(css)',
    fixtures: ['src/css/styling-v2.test.ts', 'src/css/css.out.test.ts'],
    coverage: 'typed-subset+raw',
    decision: 'Typed subset plus raw: only high-frequency descriptor rules earn dedicated APIs; the ordered raw lane prevents lossy map models.',
  }),
