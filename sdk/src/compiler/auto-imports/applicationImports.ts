...
export function resolveAppAutoImports(
  ...
  for (const source of normalized.sources) {
    ...
    const names = getExportNamesFromFile(from, root)
    sources.push({
      from,
...
export { isPackageSpecifier } from '../projection/exportNames'
...
function assertApplicationSourceDoesNotReexportStyleModule(
  ...
  const styleModule = getExportModuleFilesFromFile(resolvedSource, root)
