 ...
 * authoring code shipped ([patterns.md §1]). Locked against
...
describe('auto-imports', () => {
  it('readStyleExportNames reads every export form', () => {
    ...
    expect(readStyleExportNames(source).sort())
  ...
  it('export discovery follows syntax through multiline destructuring and defaults', () => {
    ...
    expect(readStyleExportNames(source).sort()).toEqual(['makeRecipe', 'style', 't'])
  })

  it('generates exact ambient aliases with a stable self-describing banner', () => {
    expect(renderStyleAutoImportDeclarations([{ from: '/design/authoring.ts', imports: ['ds', 'css'] }])).toBe(
