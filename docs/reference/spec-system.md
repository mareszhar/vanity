...
Vanity is a design-system engine and TypeScript harness for CSS. A system has two public states: an immutable open system accumulates capabilities, and `consolidate()` resolves an immutable locked system that styles and binds runtime behavior.
...
`createSystem()` accepts the system-wide policy record:

- CSS support policy;
- default unit behavior;
- constructor restrictions and default units;
- `tokens.reference` and `tokens.emit` defaults;
- support, layer order, plugin policy, and deterministic custom policy keys.
...
They deliberately do not expose a final custom-property name or `var()` reference because prefix, root, layers, and final token resolution belong to consolidation.
