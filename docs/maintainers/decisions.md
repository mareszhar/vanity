...
| Every compiler-owned stylesheet is addressed under the artifact directory inside the build root, and the ID-to-URL conversions are exact inverses. | Where a style source sits on disk stops being an addressing concern. |
...
| A configured system module is owned when the host loads it and is served under its own ID; hooks are natively narrowed to the IDs they serve. | Vanity's cost follows what an application uses, not the size of its graph. |
| A package whose dependencies or peer dependencies reach `@mszr/vanity` is declared to the host as a package that must pass through Vanity, and an explicit override wins with a visible warning. | An installed package behaves like a linked one, and a conflicting configuration is never silent. |
| One Vanity walk over dependency and peer edges decides and declares packages that reach `@mszr/vanity`; `vitefu` supplies package resolution, optimizer eligibility, and host-pattern matching primitives. | The role rule is identical at every depth, while package layouts, CommonJS detection, and consumer overrides use the ecosystem's shared primitives. |
