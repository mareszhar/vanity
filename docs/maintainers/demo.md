...
- **Generated auto-imports, not a mirrored API.** Nuxt-native Vue helpers stay auto-imported; the Vanity Vue helpers are a visible opt-in in `nuxt.config.ts`. `styleAutoImports` injects the exact locked `ds` value plus independent authoring helpers from a plain re-export module. Each style file locally selects its atoms (`const { class: style, t } = ds`), and Nuxt generates the exact `typeof` ambient declarations. No hand-maintained global surface exists. `ds` is auto-imported app-wide through a one-line `app/utils` re-export.
