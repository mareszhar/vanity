...
- **Lane-specific auto-imports, not a mirrored API.** Nuxt-native helpers remain Nuxt-owned; Vanity's runtime helpers are an explicit `app.runtimeAutoImports` choice in `nuxt.config.ts`. `compiler.styleAutoImports` injects the exact locked `ds` value plus independent authoring helpers into evaluated `*.css.ts` files only. Each style file locally selects its atoms (`const { class: style, t } = ds`), and Nuxt generates the exact `typeof` ambient declarations. No hand-maintained global surface exists. The app's own `app/utils` barrel remains the deliberate source for its `ds` and font exports.
