...
- **Module-role auto-imports, not a mirrored API.** Nuxt-native helpers remain Nuxt-owned; Vanity's runtime helpers are an explicit `autoImports.app` choice in `nuxt.config.ts`.
- **Shared authoring barrel.** `autoImports.shared` injects the exact locked `ds` and deliberate authoring bindings from `design/authoring.ts` into both module roles. Longer style modules use those bindings directly (`cls` for the canonical `ds.class`, `t` for tokens); concise reference examples keep the explicit `ds.class` form.
- **Generated declarations.** The studio imports its emitted typeface handles at their style-module boundary, while Nuxt generates the exact `typeof` ambient declarations and no hand-maintained app barrel exists.
