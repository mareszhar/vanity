...
| `typedGrammar`, `rawGrammar`, `inputs`, `outputType` | The structured lane, standards escape, accepted inputs, and emitted CSS kind. |
...
| Custom properties and keywords | Complete applicable lane | Compatible handles, `var()`, and CSS-wide keywords work across every declaration lane. |
...
Every applicable declaration lane accepts `initial`, `inherit`, `unset`, `revert`, and `revert-layer`. The generated matrix covers classes, nested arms, recipes, anatomy, fragments, rules, atoms, raw escapes, and token/port declarations. A helper parser cannot erase a keyword accepted by its property grammar.
...
CI fails when an exported CSS-named capability lacks a record, evidence contradicts its coverage state, a required keyword lane is missing, a pinned spec changes without review, or a documented raw escape becomes unreachable. Release does not ship a planned gap as a typed claim.
