...
An execution tracker is the implementer's own record and the thing that survives a compressed context. Everything needed to resume belongs in it rather than in recollection. It names files, commands, and decisions as they were at the time; it is not maintained after the change ships. A pass closes with one row for every item in the round report's working-tree table and the handoff's documentation list. Each row is answered with a `file:line` or a gate result, and the reviewer reads that table before anything else.

A round report reviews one execution: what was built well, what is wrong, what evidence proves or fails to prove, and what in the working tree needs realigning before the next pass. It also summarizes any change made to the handoff, so the implementer knows what moved under them. The first round audits the whole execution: every changed file is read in full, and every requirement in the handoff is checked against evidence. Later rounds review what changed since, plus the open findings. Each round gets its own report, and none is rewritten afterwards. When an initiative takes more rounds than expected, the sequence of reports is the evidence a fresh reviewer uses to see why, and it exists nowhere else. Once the release ships, its round reports have done their job, and the maintainer deletes them; the handoff, tracker, and changelog remain.
...
`__temp__/` is ignored for that reason, not by oversight. The changelog's durable home is the GitHub release that the publish flow's tag creates. Deleting anything in `__temp__/` is the maintainer's call alone: no other role removes a record or a probe, even one whose job looks done.
...
**Establish a domain before prescribing a fix.** A plan derived from a single reproduction describes that reproduction. Before writing a correction, a constant, a spelling, or a guard into a handoff, state the full set of inputs it must hold over: every caller of the function, both platforms, each shape a host can hand you, and every host the code runs in. A guard placed without its domain breaks a caller nobody enumerated. An addition made mid-release gets the same enumeration as the original plan.

**Specify the shape of a new mechanism, not only its outcome.** When a requirement needs machinery the code does not have yet, the handoff names the shape it expects, names the shapes it rules out, and states structural acceptance beside behavioral acceptance. An outcome alone invites whichever mechanism reaches it first, and a green suite cannot tell a legible mechanism from a clever one. The specification is held to the same lean-growth bar as the implementation: a mechanism, guard, or test the handoff demands must pass the test it would face if an implementer had proposed it, [testing §1.3](./testing.md#13-what-earns-a-persisted-test) included.

**Prove unproven host behavior before building on it.** When a design rests on what a host, bundler, or runtime does and nothing has shown it yet, the plan opens with a spike, and each claim states beforehand what the plan does if it fails: continue on a named fallback, or stop and raise it. The same holds for evidence: a test scenario the plan prescribes is run once as a probe before it is written down. An implementer who meets a surprise then already knows its consequence instead of improvising one.
