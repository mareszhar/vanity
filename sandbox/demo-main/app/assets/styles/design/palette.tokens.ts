...
export const paletteTokens = open.defineTokens({
  ...
  .add(({ color }) => ({
    color: {
      ...
      // the whole hue range.
      brand: open.oklch(0.60, 0.15, color.hue),
    },
  }))
  /**
  ...
  .add(({ color }) => ({
    color: {
      ...
      canvas: open.oklchx.from(color.brand, { e: 0, c: open.exact(0.012) }),
      surface: open.oklchx.from(color.brand, { e: 0.05, c: open.exact(0.014) }),
      raised: open.oklchx.from(color.brand, { e: 0.09, c: open.exact(0.018) }),
      overlay: open.oklchx.from(color.brand, { e: 0.13, c: open.exact(0.022) }),
      border: open.oklchx.from(color.brand, { e: 0.2, c: open.exact(0.026) }),
      borderStrong: open.oklchx.from(color.brand, { e: 0.32, c: open.exact(0.03) }),
      inkFaint: open.oklchx.from(color.brand, { e: 0.5, c: open.exact(0.034) }),
      inkMuted: open.oklchx.from(color.brand, { e: 0.68, c: open.exact(0.038) }),
      ink: open.oklchx.from(color.brand, { e: 0.97, c: open.exact(0.042) }),
    },
  }))
  .add(({ color }) => ({
    color: {
      ...
      brandSoft: open.mix(color.canvas, color.brand, 0.14),
      brandMuted: open.mix(color.canvas, color.brand, 0.34),
      brandHover: open.mix(color.brand, color.ink, 0.14),
      brandActive: open.mix(color.brand, color.ink, 0.24),
      // Alpha belongs here: a scrim genuinely *is* transparent.
      scrim: open.alpha(color.ink, 0.55),
      positive: open.oklch(0.68, 0.14, 155),
      warning: open.oklch(0.76, 0.15, 75),
      danger: open.oklch(0.62, 0.2, 25),
      onStatus: open.legibleOn(open.oklch(0.62, 0.2, 25)),
...
export const shadowTokens = open
  ...
  .add(({ color }) => ({
    shadow: {
      raised: open.tdef({
        val: `0 1px 2px ${color.shadowNear}, 0 4px 12px ${color.shadowFar}`,
        axes: {
          density: {
            compact: `0 1px 2px ${color.shadowNear}, 0 2px 6px ${color.shadowFar}`,
            spacious: `0 2px 4px ${color.shadowNear}, 0 12px 28px ${color.shadowFar}`,
          },
        },
        description: 'Resting surface lift; density lifts it, scheme fades it.',
      }),
      panel: open.tdef({
        val: `0 2px 6px ${color.shadowNear}, 0 16px 40px ${color.shadowFar}`,
        axes: {
          density: {
            compact: `0 1px 4px ${color.shadowNear}, 0 10px 26px ${color.shadowFar}`,
            spacious: `0 4px 10px ${color.shadowNear}, 0 28px 64px ${color.shadowFar}`,
          },
        },
